...
const Tab = ({ screens, currentScreen, setCurrentScreen }: TabProps) => {
    return (Object.keys(screens) as Screen[]).map((key: Screen) => {
        ...
        return (
            <div
                className={"pt-4 pb-23 px-3 rounded-full border-6 transition-colors duration-150 border-b-0 " + (currentScreen.toString() === key ? "bg-white border-white " : "bg-blue cursor-pointer border-blue hover:bg-light-blue ")}
